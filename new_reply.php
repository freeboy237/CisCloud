...
{
    $dn1 = ('select count(t.id) as nb1, t.title, t.parent, c.name from topics as t, categories as c where t.id="'.$id.'" and t.id2=1 and c.id=t.parent group by t.id');
...
?>
    ...
    <title>Réponse <?php echo htmlentities($dn1['name'], ENT_QUOTES, 'UTF-8'); ?></title>
    <!-- plugins:css -->
...
if(isset($_POST['message']) and $_POST['message']!='')
{
  ...
  if ($_FILES['fichier']['error']) {
    ...
    $nom_f = $_FILES['fichier']['name'];
    $nomdestination = './upload_file/file.jpg';
    move_uploaded_file($_FILES["fichier"]["tmp_name"], "upload_file/" . $_FILES["fichier"]["name"]);
  }  
    $reply = ('insert into topics (parent, id, id2, title, message, file, authorid, timestamp, timestamp2) select "'.$dn1['parent'].'", "'.$id.'", max(id2)+1, "", "'.$message.'","'.$nom_f.'", "'.$_SESSION['userid'].'", "'.time().'", "'.time().'" from topics where id="'.$id.'"');
    $reply1 = ('update topics set timestamp2="'.time().'" where id="'.$id.'" and id2=1');
    $reply = $connx->prepare($reply);
    $reply1 = $connx->prepare($reply1);
    $reply->execute();
    
	if($reply1->execute())
	{
	?>
	<div class="btn ">Le message a bien &eacute;t&eacute; envoy&eacute;.<br />
	<a href="read_topic.php?id=<?php echo $id; ?>" class="btn btn-danger">Retourner au sujet</a></div>
	<?php
	}
	else
	{
...
<form action="new_reply.php?id=<?php echo $id; ?>" method="post" class="form"  enctype="multipart/form-data">
    <label for="message"><h1><i class="mdi mdi-grease-pencil"></i>Message</h1></label><br />
