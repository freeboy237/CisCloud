...
if(isset($_GET['parent']))
{
	...
	$dn1 = (('select count(c.id) as nb1, c.name,count(t.id) as topics from categories as c left join topics as t on t.parent="'.$id.'" where c.id="'.$id.'" group by c.id'));
      ...
      <div class="container-fluid page-body-wrapper">
        ...
        <div class="main-panel">
          ...
          <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                  <li class="breadcrumb-item"><a href="#">Sujets</a></li>
                  <li class="breadcrumb-item active" aria-current="page">Liste des sujets</li>
                </ol>
              </nav>
          <?php
          $dn1 = 'select * from categories where id="'.$id.'"';
          $dn1 = $connx->prepare($dn1);
          $dn1 -> execute();
         $dn1= $dn1->fetch();
         ?>
<h1><i class="mdi mdi-server-security"></i><?php echo $dn1['name'];?></h1>
          <?php
}
if(isset($_SESSION['username']))
{
?>
	<a href="new_topics.php?parent=<?php echo $id; ?>" class="btn btn-dark">Nouveau Sujet</a>
<?php
}
$dn2 = ('select t.id, t.title, t.authorid, u.nom as author, count(r.id) as replies from topics as t left join topics as r on r.parent="'.$id.'" and r.id=t.id and r.id2!=1  left join users as u on u.id=t.authorid where t.parent="'.$id.'" and t.id2=1 group by t.id order by t.timestamp2 desc');
$dn3 = ('select t.id, t.title, t.authorid, u.avatar, u.nom as author, count(r.id) as replies from topics as t left join topics as r on r.parent="'.$id.'" and r.id=t.id and r.id2!=1  left join users as u on u.id=t.authorid where t.parent="'.$id.'" and t.id2=1 group by t.id order by t.timestamp2 desc');
$dn3 = $connx->prepare($dn3);
$dn2 = $connxx->prepare($dn2);
$dn2->execute();
$dn3->execute();
$dn2= $dn2->fetch();
if($dn2>=0)
...
<?php
while($dnn2 = $dn3->fetch())
{
?>
	<tr>
    	<td class="text-capitalize"><h5><a href="read_topic.php?id=<?php echo $dnn2['id']; ?>" class="btn btn-secondary text-info"><i class="mdi mdi-file icon-lg"></i><?php echo htmlentities($dnn2['title'], ENT_QUOTES, 'UTF-8'); ?></a></h5></td>
    	<td>
      <?php  if($dnn2['avatar']!='')
{
?>
...
?>
        <a href="profile.php?id=<?php echo $dnn2['authorid']; ?>" class="btn"><?php echo htmlentities($dnn2['author'], ENT_QUOTES, 'UTF-8'); ?></a></td>
    	<td><?php echo $dnn2['replies']; ?></td>
<?php
if(isset($_SESSION['username']) or $dn['grade']==1 or $dn['grade']==4)
{
?>
    	<td><a href="delete_topic.php?id=<?php echo $dnn2['id']; ?>"><img src="assets/images/delete.png" alt="Delete" /></a></td>
<?php
}
?>
    </tr>
