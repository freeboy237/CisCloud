...
if(isset($_POST['name'], $_POST['description']) and $_POST['name']!='')
{
	...
	{
	?>
	<div class="btn">La catégorie a bien été modifiée.<br />
	<a href="index.php" class="btn btn-danger">Retourner à l'acceuil</a></div>
	<?php
	}
	else
	{
...
{
?>
<form action="edit_category.php?id=<?php echo $id; ?>" method="post" class="form">
	<label for="name"><b>Nom</b></label><input type="text" name="name" id="name" value="<?php echo $dn2['name']; ?>" /><br />
	<label for="description"><b>Description</b></label>(html accepté)<br />
    <textarea name="description" id="description" cols="70" rows="6"><?php echo $dn2['description']; ?></textarea><br />
    <input type="submit" value="Modifier" class="btn btn-success" class="form form-control"/>
    <a href="index.php" class="btn btn-danger" style="float:right;"><i class="mdi mdi mdi mdi-keyboard-backspace"></i></a></div>
</form>
<?php
}
}
