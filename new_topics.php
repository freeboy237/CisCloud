...
{
	$dn1 = ('select count(c.id) as nb1, c.name from categories as c where c.id="'.$id.'"');
    $dn1 = $connxx->prepare($dn1);
    $dn1->execute();
    $dn1 =$dn1->fetch();
if($dn1['nb1']>0)
{
    if(isset($_POST['message'], $_POST['title']) and $_POST['message']!='' and $_POST['title']!='')
{
    ...
    $requette=$conn->prepare('insert into topics (parent, id, id2, title, message, authorid, timestamp, timestamp2) select "'.$id.'", ifnull(max(id), 0)+1, "1", "'.$title.'", "'.$message.'", "'.$_SESSION['userid'].'", "'.time().'", "'.time().'" from topics');
//	$requette->bind_param($id,$title,$message,$_SESSION['userid'],$i,$i);
    if($requette->execute())
	{
	?>
	<h1><div  class="btn btn-success">Le sujet a bien &eacute;t&eacute; créé.<br /></h1>
	<a href="list_topics.php?parent=<?php echo $id; ?>" class="btn btn-danger">Retourner a l'acceuil</a></div>
	<?php
	}
	else
	{
    ...
    <div class="container-scroller">
     ...
     <nav class="navbar default-layout-navbar col-lg-12 col-12 p-0 fixed-top d-flex flex-row">
        <div class="text-center navbar-brand-wrapper d-flex align-items-center justify-content-center">
          <a class="navbar-brand brand-logo" href="index.php"><img src="kdrive.jpg" alt="logo" /></a>
        ...
        <div class="main-panel">
          ...
          <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                  ...
                  <li class="breadcrumb-item active" aria-current="page">New Sujets Form</li>
                </ol>
              </nav>
          <form action="new_topics.php?parent=<?php echo $id; ?>" method="post" class="form">
	<label for="title">Titre</label><input type="text" name="title" id="title" class="form-control-lg" /><br />
    <label for="message">Message</label><br />
    <div class="message_buttons">
        <input class="form-control-sm" type="button" value="Gras" onclick="javascript:insert('[b]', '[/b]', 'message');" /><!--
        --><input class="form-control-sm" type="button" value="Italique" onclick="javascript:insert('[i]', '[/i]', 'message');" /><!--
       <input class="form-control-sm" type="button" value="Souligne" onclick="javascript:insert('[u]', '[/u]', 'message');" /> --><!--
        --><input class="form-control-sm" type="button" value="Image" onclick="javascript:insert('[img]', '[/img]', 'message');" /><!--
        --><input class="form-control-sm" type="button" value="Lien" onclick="javascript:insert('[url]', '[/url]', 'message');" /><!--
        --><input class="form-control-sm" type="button" value="Gauche" onclick="javascript:insert('[left]', '[/left]', 'message');" /><!--
        --><input class="form-control-sm" type="button" value="Centre" onclick="javascript:insert('[center]', '[/center]', 'message');" /><!--
        --><input class="form-control-sm" type="button" value="Droite" onclick="javascript:insert('[right]', '[/right]', 'message');" />
    </div>
    <textarea name="message" id="message" cols="70" rows="6" class="form-control-lg" ></textarea><br />
    <input type="submit" value="Envoyer" class="btn btn-block btn-lg btn-gradient-success mt-4"/>
