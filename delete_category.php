...
{
$id = intval($_GET['id']);
$dn1 = ('select count(id) as nb1, name, position from categories where id="'.$id.'" group by id');
...
?>
    ...
    <title>Supprimer une catégorie - <?php echo htmlentities($dn1['name'], ENT_QUOTES, 'UTF-8'); ?></title>
    <!-- plugins:css -->
...
$dn1 = $dn1->fetch();
    ...
    $delete1 = ('delete from topics where parent="'.$id.'"');
    $delete2 = ('update categories set position=position-1 where position>"'.$dn1['position'].'"');
    $delete = $connx->prepare($delete);
    $delete1 = $connx->prepare($delete1);
    $delete2 = $connx->prepare($delete2);
    $delete->execute();
    $delete1->execute();
	if($delete2->execute())
	{
	?>
	<div class="btn btn" style="float:right;">La catégorie et ses sujets a bien été supprimée.<br />
	<a href="<?php echo $url_home; ?>" class="btn btn-danger">Retourner à l'index du Forum</a></div></div>
	<?php
	}
	else
	{
...
{
?>
<form action="delete_category.php?id=<?php echo $id; ?>" method="post" class="form">
	Êtes-vous sûr de vouloir supprimer cette catégorie et l'ensemble de ses sujets?
    <input type="hidden" name="confirm" value="true" />
    <input type="submit" value="Oui" class="btn btn-sm btn-gradient-primary"  /> <input type="button" value="Non" onclick="javascript:history.go(-1);" class="btn btn-sm btn-gradient-primary" />
</form>
</div>
